...
export default function HomePage() {
  ...
  const overlayRef = useRef<HTMLCanvasElement>(null); // cursor overlay
  ...
  // Init canvas when resultUrl is set
  const initCanvas = useCallback(() => {
    ...
    img.src = resultUrl;
    // Load original for restore brush
    if (originalUrl) {
  ...
  const handleDownload = () => {
    ...
    if (shadow) {
      ...
      ctx.shadowBlur = Math.round(shadowOpacity/2);
      ctx.shadowOffsetY = Math.round(shadowOpacity/4);
    }
    ctx.filter = [
      brightness !== 100 ? `brightness(${brightness}%)` : "",
      contrast !== 100 ? `contrast(${contrast}%)` : "",
    ...
    a.download = `clearbg.${isPng ? "png" : "jpg"}`;
    a.click();
  };

  const reset = () => {
    setStage("idle"); setOriginalUrl(null); setResultUrl(null);
    setProgress(0); setCanvasReady(false); setShowOriginal(false);
            ...
            <motion.div key="done" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={reset} className="text-xs px-3 py-1.5 border border-white/10 rounded-lg text-white/40 hover:text-white transition-colors">
                    Nueva
                  </button>
              ...
              <div className="grid grid-cols-[1fr_300px] gap-6">
                ...
                <div className="rounded-2xl border border-white/10 overflow-auto" style={{ maxHeight: 580 }}>
                  <div
                    ...
                    style={{
                      minHeight: 400,
                      backgroundImage: bgMode === "transparent" && !blurBg ? checker : undefined,
                      ...bgStyle(),
                    }}
                  >
                    {/* BG image */}
                    ...
                    {showOriginal && originalUrl ? (
                      <img src={originalUrl} alt="Original"
                        className="relative z-10 max-w-full max-h-[520px] object-contain pointer-events-none"
                      ...
                      <div className="relative" style={{ transform: `scale(${zoom/100})`, transformOrigin: "center" }}>
                        ...
                        <canvas
                          ref={canvasRef}
                          className="max-w-full max-h-[520px] block"
                          style={{
                            cursor: activeTab === "recortar" ? "none" : "default",
                            filter: subjectFilter || "none",
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button onClick={() => setBrushMode("remove")}
                          className={`flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium border transition-all ${brushMode === "remove" ? "bg-red-500/15 border-red-500/40 text-red-400" : "border-white/8 text-white/40 hover:text-white"}`}>
                          <Minus size={14}/> Borrar
                        </button>
                        <button onClick={() => setBrushMode("add")}
                          className={`flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium border transition-all ${brushMode === "add" ? "bg-green-500/15 border-green-500/40 text-green-400" : "border-white/8 text-white/40 hover:text-white"}`}>
                          <Plus size={14}/> Restaurar
                        </button>
                      </div>
                      <div>
                        <div className="flex justify-between text-xs text-white/40 mb-2">
                          <span>Tamaño del pincel</span><span>{brushSize}px</span>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <button onClick={initCanvas}
                          className="py-2 border border-white/10 rounded-xl text-xs text-white/40 hover:text-white/70 transition-colors flex items-center justify-center gap-1">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {([["transparent","Sin fondo"],["color","Color"],["gradient","Degradado"],["image","Imagen"]] as [BgMode,string][]).map(([m,l]) => (
                          <button key={m} onClick={() => setBgMode(m)}
                            className={`py-2.5 rounded-xl text-xs font-medium border transition-all ${bgMode === m ? "bg-[#a78bfa]/15 border-[#a78bfa]/40 text-[#a78bfa]" : "border-white/8 text-white/40 hover:text-white hover:border-white/15"}`}>
                            {l}
                          </button>
                        ))}
                      </div>
                        ...
                        <div className="space-y-3">
                          <div className="grid grid-cols-6 gap-2">
                            {COLOR_PRESETS.map(({c,label}) => (
                              <button key={c} onClick={() => setBgColor(c)} title={label}
                                style={{ background: c }}
                                className={`w-9 h-9 rounded-lg border-2 transition-all ${bgColor === c ? "border-[#a78bfa] scale-110" : "border-white/10 hover:scale-105"}`} />
                            ))}
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {PRESET_GRADIENTS.map(g => (
                            <button key={g.value} onClick={() => setBgGradient(g.value)}
                              className={`h-14 rounded-xl overflow-hidden border-2 transition-all flex items-center justify-center ${bgGradient === g.value ? "border-[#a78bfa]" : "border-transparent hover:border-white/20"}`}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="p-4 bg-white/3 border border-white/8 rounded-xl space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => setShadow(v => !v)}
                            className={`w-11 h-6 rounded-full relative transition-colors ${shadow ? "bg-[#a78bfa]" : "bg-white/15"}`}>
                      ...
                      <div className="p-4 bg-white/3 border border-white/8 rounded-xl space-y-4">
                        ...
                        {[
                          ...
                          { label: "Contraste", val: contrast, set: setContrast },
                          { label: "Saturación", val: saturation, set: setSaturation },
                        ].map(a => (
                          <div key={a.label}>
                            <div className="flex justify-between text-xs text-white/40 mb-1"><span>{a.label}</span><span>{a.val}%</span></div>
                            <input type="range" min={50} max={200} step={5} value={a.val}
                              onChange={e => a.set(Number(e.target.value))} className="w-full accent-[#a78bfa]" />
                          </div>
                        ))}
