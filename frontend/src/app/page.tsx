...
import { Upload, Sparkles, Download, Zap, Shield, Layers, ArrowRight, Plus, Minus, RotateCcw, Image as ImageIcon } from "lucide-react";
...
export default function HomePage() {
  ...
  const [activeTab, setActiveTab] = useState<"edit" | "bg">("edit");
  ...
  const paint = useCallback((x: number, y: number) => {
    ...
    if (brushMode === "remove") {
      ...
      if (subjectImgRef.current) {
        ...
        ctx.arc(x, y, brushSize, 0, Math.PI * 2);
        ctx.clip();
        ctx.drawImage(subjectImgRef.current, 0, 0);
      }
    }
    ctx.restore();
  }, [brushMode, brushSize]);

  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => { setIsDrawing(true); paint(...Object.values(getCanvasPos(e)) as [number,number]); };
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => { if (!isDrawing) return; paint(...Object.values(getCanvasPos(e)) as [number,number]); };
  const handleMouseUp = () => setIsDrawing(false);

  const handleReset = () => {
    const canvas = canvasRef.current!;
    const ctx = canvas.getContext("2d")!;
    if (subjectImgRef.current) { ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.drawImage(subjectImgRef.current, 0, 0); }
  };
  ...
  const getPreviewStyle = (): React.CSSProperties => {
    ...
    return { backgroundImage: `url("data:image/svg+xml,%3Csvg width='20' height='20' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='10' height='10' fill='%23333'/%3E%3Crect x='10' y='10' width='10' height='10' fill='%23333'/%3E%3Crect x='10' y='0' width='10' height='10' fill='%23222'/%3E%3Crect x='0' y='10' width='10' height='10' fill='%23222'/%3E%3C/svg%3E")` };
  };
  ...
  const handleDownload = () => {
    ...
    if (bgMode === "color") { ctx.fillStyle = bgColor; ctx.fillRect(0, 0, offscreen.width, offscreen.height); }
    else if (bgMode === "gradient") {
      const match = bgGradient.match(/#[0-9a-f]{3,8}/gi);
      if (match && match.length >= 2) {
        const grd = ctx.createLinearGradient(0, 0, offscreen.width, offscreen.height);
        grd.addColorStop(0, match[0]); grd.addColorStop(1, match[1]);
        ctx.fillStyle = grd; ctx.fillRect(0, 0, offscreen.width, offscreen.height);
      }
    }
    ...
    a.href = offscreen.toDataURL(mime, 0.95);
    a.download = `clearbg-resultado.${ext}`;
    a.click();
  };

  const reset = () => { setStage("idle"); setOriginalUrl(null); setResultUrl(null); setProgress(0); setBgMode("transparent"); };
              ...
              <div className="grid grid-cols-[1fr_320px] gap-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-white/50">Resultado — arrastrá para ver el antes/después</p>
                  ...
                  <div className="rounded-2xl overflow-hidden border border-white/10 relative" style={getPreviewStyle()}>
                    ...
                    {showOriginal && originalUrl ? (
                      <img src={originalUrl} alt="Original" className="w-full max-h-[500px] object-contain relative z-10" />
                    ) : (
                      <canvas
                        ref={canvasRef}
                  ...
                  {/* Brush toolbar */}
                  <div className="flex items-center gap-4 p-3 bg-white/3 border border-white/8 rounded-xl">
                    ...
                    <button onClick={handleReset} className="text-xs px-3 py-1.5 border border-white/10 rounded-lg text-white/40 hover:text-white/70 transition-colors flex items-center gap-1">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {PRESET_GRADIENTS.map(g => (
                        <button key={g.value} onClick={() => setBgGradient(g.value)}
                          className={`relative h-12 rounded-xl overflow-hidden border-2 transition-all ${bgGradient === g.value ? "border-[#a78bfa]" : "border-transparent hover:border-white/20"}`}
